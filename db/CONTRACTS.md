...
- `contact`: `{ phoneDisplay:"(11) 98706-5632", phoneTel:"+5511987065632", whatsappNumber:"5511987065632",
  ...
  mapEmbed:"<iframe src do Google Maps atual>",
  reviewCount?:"N", ratingValue?:"5.0" }`
  — `reviewCount`/`ratingValue` vivem **só na D1** (atualização frequente); seed/código não
  espelham o número para não ficar desatualizado. UI e JSON-LD só exibem se presentes.
...
- **principles** → `principles-section.tsx` (export `ConditionsTreated`). content `{ title, subtitle, items:[{name,description,focus,image?,imageAlt?}] }`; design `{ columns:3, numbered, showFocus, background }`. Imagens em `public/images/principles/`.
